...
public class AltaCompaniaSeguro extends GenericJFrame {
	...
	protected void inicializar() {
		...
		setTitle("Alta Compania Seguro");
	}

	protected void configurar() {
		JPanel panelCentro = new JPanel();
		panelCentro.setLayout(new GridLayout(0, 1));
		String[] labelStrings = { "CUIL", "Nombre" };
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnAgregar) {
			if (!hayCampoVacio()) {
				int i = 0;
				String cuil = textFields[i++].getText();
				String nombre = textFields[i++].getText();

				try {
					CompaniaSeguroView cs = new CompaniaSeguroView(cuil, nombre);
					BusinessDelegate.getInstance().getInterfaz().altaCompaniaSeguro(cs);
					mostrarInformacion("Compania seguro agregada correctamente.");
				} catch (Exception ex) {
