...
public class AltaCarga extends GenericHttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			Integer idSucOrigen = request.getParameter("idSucursalOrigen") != null ? Integer.parseInt(request.getParameter("idSucursalOrigen"))
					: null;
			...
			Integer idSucDestino = request.getParameter("idSucursalDestino") != null ? Integer.parseInt(request.getParameter("idSucursalDestino"))
					: null;
			...
			Boolean exclusiva = request.getParameter("exclusiva") != null ? Boolean.parseBoolean(request.getParameter("retira")) : null;
			List<ItemProductoJson> productosJson = null;
			if (request.getParameter("productos") != null) {
				Gson gson = new Gson();
				Type tipo = new TypeToken<List<ItemProductoJson>>() {
				}.getType();
				productosJson = gson.fromJson((String) request.getParameter("productos"), tipo);
			}
			UbicacionView destino = null;
			if (idSucDestino != null) {
				SucursalView sucDestino = BusinessDelegate.getInstance().getInterfaz().obtenerSucursal(idSucDestino);
			...
			Integer id = BusinessDelegate.getInstance().getInterfaz().altaCarga(idSucOrigen, idCliente, carga, !local, exclusiva);
			String fechaProbable = Utilities.invParseDate(BusinessDelegate.getInstance().getInterfaz().fechaProbableLlegada(id));
